...
/// 系统登录授权服务 🧩
/// </summary>
[ApiDescriptionSettings(Order = 500, Groups = new[] { AppCoreConst.GroupSystemApi })]
public class SysAuthService : IDynamicApiController, ITransient
{
    ...
    public virtual async Task<LoginOutput> Login([Required] LoginInput input)
    {
        ...
        try
        {
            if(input.AccountType == AccountTypeEnum.Member && Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") != "Development") { } else
            {
        ...
        // 是否开启域登录验证
        if (input.AccountType != AccountTypeEnum.Member && await _sysConfigService.GetConfigValue<bool>(CommonConst.SysDomainLogin))
        {
    ...
    internal virtual async Task<LoginOutput> CreateToken(SysUser user)
    {
        ...
        if (user.AccountType == AccountTypeEnum.Member)
        {
            var tenant = await _sysUserRep.ChangeRepository<SqlSugarRepository<SysTenant>>().AsQueryable().Where(x => x.Id == user.TenantId).FirstAsync();
            payload.Add(ClaimConst.GameAccountPrefix, tenant.Code);
        }
        var accessToken = JWTEncryption.Encrypt(payload, tokenExpire);
...
public class SpecificationAuth
{
    /// <summary>
    /// 用户名
    /// </summary>
