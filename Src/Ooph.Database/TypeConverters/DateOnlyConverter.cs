...
public class DateOnlyConverter : ISugarDataConverter
{
    ...
    public T QueryConverter<T>(IDataRecord dr, int i)
    {
        ...
        var strDateOnly = str.Split(' ')[0].Replace("/", "-");
        var val = DateOnly.Parse(strDateOnly);
        var json = System.Text.Json.JsonSerializer.Serialize(val);
        //return System.Text.Json.JsonSerializer.Deserialize<T>(json);
        return new SerializeService().DeserializeObject<T>(json);
    }
}